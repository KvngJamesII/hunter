...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    // Handle client messages (like user authentication)
  ...
  app.post("/api/notifications/register-device", async (req: Request, res: Response) => {
    try {
      const { userId, token, platform, deviceInfo } = z.object({
        userId: z.number(),
        token: z.string(),
        platform: z.string(),
      ...
      // Here we would normally store the device token in the database
      // For now, we'll just log it and return success
      console.log(`Registered device for user ${userId}: ${token}`);
  ...
  app.post("/api/notifications/send", async (req: Request, res: Response) => {
    try {
      const { userId, title, body, data } = z.object({
        userId: z.number(),
        title: z.string(),
        body: z.string(),
      ...
      // Use our WebSocket function to send real-time notification
  ...
  app.get("/api/admin/logs", async (req: Request, res: Response) => {
    try {
      ...
      // Simulated admin logs - in a real app this would come from database
      const logs = [
        {
          id: 1,
          adminId: 1,
          adminEmail: "mike@gmail.com",
          action: "login",
          targetType: "user",
          targetId: 1,
        ...
        },
        {
          ...
          targetId: 1,
          details: "Approved deposit of ₦5,000 from user danny@gmail.com",
        ...
        },
        {
          ...
          targetType: "user",
          targetId: 2,
          details: "New user registration: danny@gmail.com",
        ...
        },
        {
          ...
          targetType: "user", 
          targetId: 3,
          details: "New user registration: stop@gmail.com",
